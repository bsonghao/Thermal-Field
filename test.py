...
fock_mo = np.einsum('pi,pq,qj->ij', myhf.mo_coeff, fock_ao, myhf.mo_coeff)
...
model = two_body_model(E_0, fock_mo, S_ovlp_mo, eri_mo, n_el)
# thermal field transform
model.thermal_field_transform(T=3e5)
# TFCC imaginary time integration
model.thermal_field_coupled_cluster(T_final=1e8, N=1000)
