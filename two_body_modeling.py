...
class two_body_model():
    ...
    def __init__(self, name, E_core, H_core, Fock, V_eri, n_occ, molecule, E_NN, T_2_flag=True, chemical_potential=True, partial_trace_condition=True):
        """
   ...
   def _check_P_Q_G_condition(self, RDM_1, T_2):
    ...
    def TFCC_integration(self, T_final, N, direct_flag=True, exchange_flag=True, constraint_flag=True):
        ...
        self.L_lambda = []
        # initial P, Q, G plots
        self.P_Q_G_condition_alpha_beta = {"T(K)": self.T_grid, "P": [], "Q": [], "G": []} # alpha beta component
        self.P_Q_G_condition_alpha_alpha = {"T(K)": self.T_grid, "P": [], "Q": [], "G": []} # alpha beta component
        ...
        for i in range(N):
            ...
            if i != 0:
                ...
                if constraint_flag:
                    ...
                    n_occ, orb = np.linalg.eig(RDM_1)
                    occupation_number_correct = np.sort(n_occ.real)
                    print("occupation number:\n{:}".format(occupation_number_correct))
        ...
        if self.T_2_flag:
            if constraint_flag:
                df.to_csv("{:}_trace_condition_TFCC_CCSD_sym_cons_new.csv".format(self.name), index=False)
            else:
                df.to_csv("{:}_trace_condition_TFCC_CCSD_sym_nocons_new.csv".format(self.name), index=False)
        ...
        if self.T_2_flag:
            if constraint_flag:
                df.to_csv("{:}_P_Q_G_condition_TFCC_CCSD_sym_cons_new._alpha_beta.csv".format(self.name), index=False)
            else:
                df.to_csv("{:}_P_Q_G_condition_TFCC_CCSD_sym_nocons_new_alpha_beta.csv".format(self.name), index=False)

        else:
            df.to_csv("{:}_P_Q_G_condition_TFCC_CCS_sym_new_alpha_beta.csv".format(self.name), index=False)

        df = pd.DataFrame(self.P_Q_G_condition_alpha_alpha)
        if self.T_2_flag:
            if constraint_flag:
                df.to_csv("{:}_P_Q_G_condition_TFCC_CCSD_sym_cons_new._alpha_alpha.csv".format(self.name), index=False)
            else:
                df.to_csv("{:}_P_Q_G_condition_TFCC_CCSD_sym_nocons_new_alpha_alpha.csv".format(self.name), index=False)

        else:
            df.to_csv("{:}_P_Q_G_condition_TFCC_CCS_sym_new_alpha_alpha.csv".format(self.name), index=False)
        ...
        if self.T_2_flag:
            if constraint_flag:
                df.to_csv("{:}_thermal_properties_TFCC_CCSD_sym_cons_new.csv".format(self.name), index=False)
            else:
                df.to_csv("{:}_thermal_properties_TFCC_CCSD_sym_nocons_new.csv".format(self.name), index=False)

        else:
            df.to_csv("{:}_thermal_properties_TFCC_CCS_sym_new.csv".format(self.name), index=False)
        ...
        if self.T_2_flag:
            if constraint_flag:
                df.to_csv("{:}_occupation_number_TFCC_CCSD_sym_cons_new.csv".format(self.name), index=False)
            else:
                df.to_csv("{:}_occupation_number_TFCC_CCSD_sym_nocons_new.csv".format(self.name), index=False)

        else:
            df.to_csv("{:}_occupation_number_TFCC_CCS_sym_new.csv".format(self.name), index=False)
        return
